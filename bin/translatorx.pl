...
# Make a file that will send instructions to Clustal with appropriate name of input file
	...
	if ($pauser eq "clustalw") {
		...
		chdir("$path/tmp");
		open(LOG, ">$out_file.$pauser.log") || die "Cannot create file outfile.pauser.log: $out_file.$pauser.log\n";
		my $pid = open(PH, "t_coffee -infile=$muscleinfile -output=pir_aln -outfile=$clustaloutfile 2>&1 |");              # or with an open pipe
		while(<PH>) {
			print STDERR $_;
			print LOG $_;
		}
		close(LOG);
		#system "t_coffee -infile=$muscleinfile -output=pir_aln -outfile=$clustaloutfile";#-outfile=$t_coffeeoutname";
		print STDERR "t_coffee -infile=$muscleinfile -output=pir_aln -outfile=$clustaloutfile\n";#-outfile=$t_coffeeoutname";
		chdir($dir);
	}
